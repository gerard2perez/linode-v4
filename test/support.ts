...
describe('Support', function () {
	...
	it('tikets', async function () {
		...
		assert.equal(await app.support.tickets(1).attachments({}), '/v4/support/tickets/1/attachments#post%data');
		assert.equal(await app.support.tickets(1).replies({}), '/v4/support/tickets/1/replies#post%data');
	});
});
